  ...
  test('derlenmiş paketlerde sunucu sırları bulunmuyor', async () => {
    ...
    for (const parca of parcalar) {
      ...
      expect(code, `${parca} sunucu sırrı içeriyor`)
        .not.toMatch(/NETGSM_PASS|OTP_SECRET|service_role_key|IYS_PASSWORD|CRON_SECRET|PARASUT_CLIENT_SECRET|PARASUT_PASSWORD|WHATSAPP_APP_SECRET|WHATSAPP_TOKEN|WHATSAPP_VERIFY_TOKEN/i);
    }
  });
});
