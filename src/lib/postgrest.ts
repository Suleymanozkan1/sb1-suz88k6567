...
class Sorgu<T> implements PromiseLike<Sonuc<T>> {
   ...
   *
   * PostgREST'te bu `Prefer: resolution=merge-duplicates` ile yapılıyor;
   * çatışma sütununu birincil anahtar ya da benzersiz kısıt belirliyor.
   */
  upsert(satirlar: unknown): this {
    this.yontem = 'POST';
    this.govde = satirlar;
...
export interface PostgrestIstemci {
  from<T = unknown>(tablo: string): Sorgu<T>;
