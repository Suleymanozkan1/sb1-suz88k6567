...
function toReservation(row: Row): Reservation {
  return {
    ...
    brideName: (row.bride_name as string) ?? undefined,
    bridePhone: (row.bride_phone as string) ?? undefined,
    groomHometown: (row.groom_hometown as string) ?? undefined,
    brideHometown: (row.bride_hometown as string) ?? undefined,
    groomName: (row.groom_name as string) ?? undefined,
    groomPhone: (row.groom_phone as string) ?? undefined,
    groomEmail: (row.groom_email as string) ?? undefined,
    groomDistrict: (row.groom_district as string) ?? undefined,
    brideEmail: (row.bride_email as string) ?? undefined,
    brideDistrict: (row.bride_district as string) ?? undefined,
    staffId: (row.staff_id as string) ?? undefined,
    staffEmail: (row.staff_email as string) ?? undefined,
    contractDate: (row.contract_date as string) ?? undefined,
    homePhone: (row.home_phone as string) ?? undefined,
    menuNote: (row.menu_note as string) ?? undefined,
    pricePerPerson: Number(row.price_per_person ?? 0),
    discount: Number(row.discount ?? 0),
    discountIsPercent: Boolean(row.discount_is_percent),
...
function fromReservation(r: Reservation) {
  return {
    ...
    groom_hometown: r.groomHometown || null,
    bride_hometown: r.brideHometown || null,
    groom_name: r.groomName || null,
    groom_phone: r.groomPhone || null,
    groom_email: r.groomEmail || null,
    groom_district: r.groomDistrict || null,
    bride_email: r.brideEmail || null,
    bride_district: r.brideDistrict || null,
    staff_id: r.staffId || null,
    staff_email: r.staffEmail || null,
    contract_date: r.contractDate || null,
    home_phone: r.homePhone || null,
    menu_note: r.menuNote || null,
    price_per_person: r.pricePerPerson ?? 0,
    discount: r.discount ?? 0,
    discount_is_percent: r.discountIsPercent ?? false,
    vat_rate: r.vatRate ?? 0,
