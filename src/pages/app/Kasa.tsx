...
type KasaSatiri =
  | {
      ...
      kaynak: 'elle'; entry: CashFlowEntry;
    }
  | {
      id: string; date: string; kind: CashFlowKind; category: string;
      description: string; amount: number; method?: PaymentMethod;
...
export default function Kasa() {
  ...
  const [form, setForm] = useState({
    ...
    amount: '',
    // Paranın hangi kanaldan girdiği/çıktığı. Kasa dağılımı buna bakıyor.
  ...
  return (
    <QueryBoundary isLoading={isLoading} error={loadError}>
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-brand-muted">
            ...
            burada kendiliğinden görünür; düzeltme rezervasyon ekranından yapılır. Paranın hangi
            kanalda durduğu ödeme tipinden çıkar, ayrıca işaretlemek gerekmez.
          </p>
        </div>
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        />
        {/*
          Kasa dağılımı burada perdesiz: bu ekranda zaten bütün satırlar
          tek tek görünüyor, toplamı gizlemek anlamsız olurdu.
        */}
        <KasaDagilimKarti dagilim={dagilim} currency={currency} />
      </div>

      {can('kasa.duzenle') && (
        <form onSubmit={(e) => { void submit(e); }} noValidate className="card mb-6 grid gap-3 p-4 md:grid-cols-2 lg:grid-cols-7">
          <div>
          ...
          <div>
            <label htmlFor="cf-method" className="field-label">Ödeme tipi</label>
            <select id="cf-method" className="field-input" value={form.method}
          ...
          {error && <p className="text-xs text-danger md:col-span-2 lg:col-span-7" role="alert">{error}</p>}
        </form>
      )}
          ...
          <table className="w-full min-w-[640px] text-sm">
            ...
            <thead>
              <tr className="border-b border-line bg-surface text-left text-xs uppercase text-brand-muted">
                ...
                <th className="px-4 py-3 font-medium">Ödeme tipi</th>
            ...
            <tbody>
              {filtered.map((e) => (
                <tr key={e.id} className="border-b border-line/60 last:border-0 hover:bg-surface/60">
                  ...
                  <td className="px-4 py-3 text-brand-muted">
                    {/* Eski kayıtların tipi bilinmiyor; uydurulmuyor. */}
